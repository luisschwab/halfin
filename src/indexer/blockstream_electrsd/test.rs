...
//! Configuration and runtime integration tests for [`BlockstreamElectrsD`].
...
#[cfg(feature = "bitcoind")]
use super::BlockstreamElectrsD;
use super::BlockstreamElectrsDConf;
#[cfg(feature = "bitcoind")]
use super::blockstream_electrs_header_matches;
use super::blockstream_electrs_header_matches_with;
use super::get_blockstream_electrs_path;
...
#[cfg(feature = "bitcoind")]
fn assert_esplora_tip(blockstream_electrs: &BlockstreamElectrsD, bitcoind: &BitcoinD) {
    ...
    assert!(blockstream_electrs.get_esplora_socket().ip().is_loopback());
    assert_eq!(
        blockstream_electrs.get_esplora_url(),
        format!("http://{}", blockstream_electrs.get_esplora_socket())
    );
    ...
    while start.elapsed() < INDEXING_TIMEOUT {
        ...
        blockstream_electrs.trigger().unwrap();
...
fn assert_esplora_transaction_status(
    blockstream_electrs: &BlockstreamElectrsD,
    txid: Txid,
    confirmed: bool,
...
#[test]
fn blockstream_electrsd_validates_binary_path_and_start_attempts() {
    let node = FakeNode::new(Network::Regtest, serde_json::json!({ "blocks": 1 }));

    let error = BlockstreamElectrsD::from_bin("electrs", &node).unwrap_err();
    assert!(matches!(error, Error::BinaryPathNotAbsolute { .. }));

    let root = tempfile::tempdir().unwrap();
    let error =
        BlockstreamElectrsD::from_bin(root.path().join("missing-electrs"), &node).unwrap_err();
    assert!(matches!(error, Error::BinaryPathNotFile { .. }));

    node.write_cookie("user:password");
    let config = BlockstreamElectrsDConf {
        max_retries: 0,
        ..BlockstreamElectrsDConf::default()
    };
    let error = BlockstreamElectrsD::from_bin_with_conf(
        get_blockstream_electrs_path().unwrap(),
        &node,
        &config,
    )
    .unwrap_err();
...
#[cfg(unix)]
#[test]
fn blockstream_electrsd_reports_test_program_startup_failures() {
    let node = FakeNode::new(Network::Regtest, serde_json::json!({ "blocks": 1 }));
    node.write_cookie("user:password");
    ...
    assert!(matches!(
        BlockstreamElectrsD::from_bin_with_conf(&program, &node, &config),
        Err(Error::Io(_))
    ));
...
#[test]
fn blockstream_electrsd_rejects_pruned_backends() {
    let node = FakeNode::new(Network::Regtest, serde_json::json!({ "blocks": 1 }))
        .with_prune(PruneMode::Automatic(1));
    node.write_cookie("user:password");

    assert!(matches!(
        BlockstreamElectrsD::from_bin(get_blockstream_electrs_path().unwrap(), &node),
...
#[test]
fn blockstream_electrsd_classifies_history_failures() {
    let script = ScriptBuf::new();
    let txid = Txid::all_zeros();

    let (client, server) = scripted_electrum_client(None);
    assert!(!BlockstreamElectrsD::script_history_has_mempool_tx(&client, &script, txid).unwrap());
    server.join().unwrap();
    ...
    assert!(matches!(
        BlockstreamElectrsD::script_history_has_mempool_tx(&client, &script, txid),
...
#[cfg(unix)]
#[test]
fn blockstream_electrsd_reports_client_setup_failures() {
    let listener = TcpListener::bind((std::net::Ipv4Addr::LOCALHOST, 0)).unwrap();
    let socket = listener.local_addr().unwrap();
    drop(listener);
    ...
    process.wait().unwrap();
    assert!(matches!(
        BlockstreamElectrsD::wait_for_client(socket, &mut process, Duration::from_secs(1)),
...
#[test]
fn blockstream_electrsd_treats_socket_timeouts_as_incomplete_reads() {
    for kind in [
    ...
    assert!(matches!(
        ...
        IndexerError::UnresponsiveIndexer {
            indexer: "BlockstreamElectrsD",
            ..
        }
    ));
}

/// Verify `Blockstream/electrs` header matching without a running server.
#[test]
fn blockstream_electrsd_matches_header_notifications() {
    let header = genesis_block(Network::Regtest).header;
    let notification = HeaderNotification { height: 1, header };

    assert!(
        !blockstream_electrs_header_matches_with(&notification, 2, None, |_| {
    ...
    assert!(
        blockstream_electrs_header_matches_with(
            &notification,
            1,
    ...
    assert!(
        blockstream_electrs_header_matches_with(&notification, 0, None, |height| {
            assert_eq!(height, 0);
            Ok(header)
        })
        .unwrap()
    ...
    assert!(
        !blockstream_electrs_header_matches_with(&notification, 0, None, |_| Err(incomplete))
            .unwrap()
    );

    let error = blockstream_electrs_header_matches_with(&notification, 0, None, |_| {
...
#[test]
fn blockstream_electrsd_rejects_oversized_notification_height() {
    let header = genesis_block(Network::Regtest).header;
    ...
    let error = blockstream_electrs_header_matches_with(&notification, 0, None, |_| Ok(header))
        .unwrap_err();
    assert!(matches!(error, Error::UnexpectedResponse(_)));
}

/// Verify that [`BlockstreamElectrsD`] accepts Bitcoin Core and serves Electrum and Esplora.
#[cfg(feature = "bitcoind")]
#[test]
#[allow(clippy::too_many_lines)]
fn blockstream_electrsd_accepts_bitcoind() {
    ...
    let mut blockstream_electrs = BlockstreamElectrsD::new(&bitcoind).unwrap();
    assert_eq!(bitcoind.get_peer_count().unwrap(), 0);
    ...
    blockstream_electrs
        ...
        .unwrap();
    assert_esplora_tip(&blockstream_electrs, &bitcoind);

    let tip = blockstream_electrs
        .client
    ...
    assert!(
        blockstream_electrs_header_matches(
    ...
    assert!(matches!(
        BlockstreamElectrsD::wait_for_client(
            blockstream_electrs.get_electrum_socket(),
            &mut blockstream_electrs.process,
            Duration::ZERO,
        ),
        Err(Error::ClientSetupTimeout)
    ));

    blockstream_electrs.client.ping().unwrap();
    assert!(blockstream_electrs.get_pid() > 0);
    assert!(blockstream_electrs.get_working_directory().is_dir());
    assert!(blockstream_electrs.get_electrum_socket().ip().is_loopback());
    assert_eq!(
        blockstream_electrs.get_electrum_url(),
        blockstream_electrs.get_electrum_socket().to_string()
    );
    assert!(
        blockstream_electrs
            .get_monitoring_socket()
            .ip()
            .is_loopback()
    );
    ...
    blockstream_electrs.client =
        electrum_client::raw_client::RawClient::new(socket, Some(Duration::from_secs(1)), None)
            .unwrap();
    assert!(matches!(
        blockstream_electrs.wait_until_block(height, None, Some(Duration::from_secs(1))),
        Err(Error::Indexer(IndexerError::UnresponsiveIndexer { .. }))
    ));
    server.join().unwrap();
    ...
    blockstream_electrs.client =
        electrum_client::raw_client::RawClient::new(socket, Some(Duration::from_secs(1)), None)
            .unwrap();
    let result =
        blockstream_electrs.wait_until_block(height, None, Some(Duration::from_millis(250)));
    release.send(()).unwrap();
    server.join().unwrap();
    assert!(matches!(
        result,
    ...
    blockstream_electrs.client =
        electrum_client::raw_client::RawClient::new(socket, Some(Duration::from_secs(1)), None)
            .unwrap();
    assert!(matches!(
        blockstream_electrs.wait_until_block(height + 1, None, Some(Duration::from_millis(250))),
        Err(Error::Indexer(IndexerError::IndexingTimeout { .. }))
    ));
    server.join().unwrap();
    ...
    blockstream_electrs.client =
        electrum_client::raw_client::RawClient::new(socket, Some(Duration::from_secs(1)), None)
            .unwrap();
    assert!(matches!(
        blockstream_electrs.wait_until_block(height + 1, None, Some(Duration::from_secs(1))),
        Err(Error::Indexer(IndexerError::UnresponsiveIndexer { .. }))
    ));
    server.join().unwrap();
}
...
#[cfg(feature = "btcd")]
#[test]
fn blockstream_electrsd_rejects_btcd() {
    let btcd = BtcD::new().unwrap();
    let temporary_directory = tempfile::tempdir().unwrap();
    let directory = temporary_directory.path().join("blockstream-electrs");
    let config = BlockstreamElectrsDConf {
    ...
    assert!(matches!(
        BlockstreamElectrsD::new_with_conf(&btcd, &config),
...
#[cfg(feature = "utreexod")]
#[test]
fn blockstream_electrsd_rejects_utreexod() {
    ...
    assert!(matches!(
        BlockstreamElectrsD::new_with_conf(&utreexod, &config),
        Err(Error::Indexer(IndexerError::UnsupportedBackend {
            node: "UtreexoD"
        }))
    ));
...
#[cfg(feature = "florestad")]
#[test]
fn blockstream_electrsd_rejects_florestad() {
    ...
    assert!(matches!(
        BlockstreamElectrsD::new_with_conf(&florestad, &config),
        Err(Error::Indexer(IndexerError::UnsupportedBackend {
            node: "FlorestaD"
        }))
    ));
...
#[cfg(feature = "bitcoind")]
#[test]
fn blockstream_electrsd_sees_mempool_transactions() {
    let bitcoind = BitcoinD::new().unwrap();
    bitcoind.generate(MATURE_COINBASE_BLOCK_COUNT).unwrap();
    let blockstream_electrs = BlockstreamElectrsD::new(&bitcoind).unwrap();
    ...
    blockstream_electrs.trigger().unwrap();
    assert!(matches!(
        blockstream_electrs.wait_until_mempool_tx(&script_pubkey, txid, Some(Duration::ZERO)),
        Err(Error::Indexer(IndexerError::IndexingTimeout { .. }))
    ));
    blockstream_electrs
    ...
    assert_esplora_transaction_status(&blockstream_electrs, txid, false, None);
    ...
    blockstream_electrs
        .wait_until_caught_up(&bitcoind, None)
        .unwrap();
    assert_esplora_transaction_status(&blockstream_electrs, txid, true, Some(confirmation_hash));
}

/// Verify repeated synchronization through both Electrum and Esplora.
#[cfg(feature = "bitcoind")]
#[test]
fn blockstream_electrsd_syncs_blocks() {
    let bitcoind = BitcoinD::new().unwrap();
...
/// Verify Electrum and Esplora follow a replacement chain tip.
#[cfg(feature = "bitcoind")]
#[test]
fn blockstream_electrsd_reindexes_reorgs() {
    let bitcoind = BitcoinD::new().unwrap();
    ...
    assert_esplora_tip(&indexer, &bitcoind);
    ...
    indexer.wait_until_caught_up(&bitcoind, None).unwrap();
    let tip = indexer.client.block_headers_subscribe().unwrap();
...
#[test]
fn blockstream_electrsd_configuration_defaults() {
    let config = BlockstreamElectrsDConf::default();

    assert!(config.raw_args.is_empty());
    assert_eq!(config.max_retries, SPAWN_ATTEMPTS);
    assert_eq!(
        BlockstreamElectrsD::configured_args(&config, Network::Regtest).unwrap(),
        ["--network", "regtest"]
    );
}

#[test]
fn blockstream_electrsd_renders_every_network() {
    let cases = [
    ...
    for (network, expected) in cases {
        assert_eq!(
            BlockstreamElectrsD::configured_args(&BlockstreamElectrsDConf::default(), network)
                .unwrap(),
            ["--network", expected]
        );
    }
...
#[test]
fn blockstream_electrsd_rejects_owned_raw_arguments() {
    let cases = [
        "--network",
        "--network=signet",
        "--db-dir",
    ...
    for arg in cases {
        ...
        assert!(matches!(
            BlockstreamElectrsD::configured_args(&config, Network::Regtest),
...
#[test]
fn blockstream_electrsd_accepts_unmodeled_raw_arguments() {
    let config = BlockstreamElectrsDConf {
        raw_args: vec![
    ...
    assert!(BlockstreamElectrsD::configured_args(&config, Network::Regtest).is_ok());
}

/// Verify process state, sockets, shutdown, and temporary cleanup.
#[cfg(feature = "bitcoind")]
#[test]
fn blockstream_electrsd_lifecycle_exposes_runtime_state_and_removes_temporary_directory() {
    let bitcoind = BitcoinD::new().unwrap();
    let config = BlockstreamElectrsDConf::default();
    let mut blockstream_electrs = BlockstreamElectrsD::new_with_conf(&bitcoind, &config).unwrap();
    let directory = blockstream_electrs.get_working_directory();

    assert!(blockstream_electrs.get_pid() > 0);
    assert!(directory.is_dir());
    assert_eq!(blockstream_electrs.get_config(), &config);
    assert!(blockstream_electrs.get_electrum_socket().ip().is_loopback());
    assert!(
        blockstream_electrs
            .get_monitoring_socket()
    ...
    assert_ne!(
        blockstream_electrs.get_electrum_socket(),
        blockstream_electrs.get_monitoring_socket()
    );
    assert_ne!(
        blockstream_electrs.get_electrum_socket(),
        blockstream_electrs.get_esplora_socket()
    );
    assert_esplora_tip(&blockstream_electrs, &bitcoind);

    blockstream_electrs.stop().unwrap();
    assert!(matches!(
        blockstream_electrs.trigger(),
        Err(Error::UnexpectedResponse(_))
    ));
    drop(blockstream_electrs);
    assert!(!directory.exists());
}
...
#[cfg(feature = "bitcoind")]
#[test]
fn blockstream_electrsd_static_directory_restores_indexed_state() {
    let bitcoind = BitcoinD::new().unwrap();
