...
//! Build local release archives for `mempool/electrs`.
//!
//! Run this Cargo example from the repository root:
//!
//! ```text
//! cargo run --example compile-mempool-electrs
//! ```
//!
//! The program gets the specified mempool/electrs release and builds each supported target.
...
// `mempool/electrs` release that the crate downloads.
include!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/src/indexer/mempool_electrsd/versions.rs"
));
...
/// `mempool/electrs` repository used as the release source.
const MEMPOOL_ELECTRS_REPO: &str = "https://github.com/mempool/electrs";
...
/// All binaries published for a single mempool/electrs release.
const TARGETS: &[Target] = &[
    Target {
        triple: "aarch64-apple-darwin",
        artifact_name: "mempool-electrs-darwin-arm64.tar.gz",
        exe_name: "electrs",
    ...
    Target {
        ...
        artifact_name: "mempool-electrs-darwin-amd64.tar.gz",
        exe_name: "electrs",
        builder: Builder::Cargo,
        bindgen_args: None,
    },
    Target {
        ...
        artifact_name: "mempool-electrs-linux-amd64.tar.gz",
        exe_name: "electrs",
    ...
    Target {
        ...
        artifact_name: "mempool-electrs-linux-arm64.tar.gz",
        exe_name: "electrs",
        builder: Builder::Cross,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let compile_mempool_electrs_dir = manifest_dir.join("contrib/bins/compile_mempool_electrs");
    let cross_config = compile_mempool_electrs_dir.join("Cross.toml");
    let dist_dir = compile_mempool_electrs_dir
        .join("dist")
        .join(format!("mempool-electrs-{MEMPOOL_ELECTRS_VERSION}"));
    let workdir = compile_mempool_electrs_dir.join("tmp");
    let source_dir = workdir.join("electrs");

    // Fail up front with actionable messages before spending time cloning or
    // compiling the mempool/electrs checkout.
    ...
    println!("mempool/electrs {} artifacts:", MEMPOOL_ELECTRS_VERSION);
    for target in TARGETS {
...
fn prepare_source(sh: &Shell, source_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if source_dir.exists() {
        log_step(format!(
            "using existing mempool/electrs checkout {}",
            source_dir.display()
        ));
    } else {
        log_step(format!(
            "cloning mempool/electrs {} into {}",
            MEMPOOL_ELECTRS_VERSION,
            source_dir.display()
        ));
        let repo = MEMPOOL_ELECTRS_REPO;
        cmd!(sh, "git clone {repo} {source_dir_s}").run_echo()?;
    }

    let sh = sh.with_current_dir(source_dir);
    log_step("fetching mempool/electrs tags");
    cmd!(sh, "git fetch --tags --force").run_echo()?;

    let tag = format!("v{MEMPOOL_ELECTRS_VERSION}");
    log_step(format!("checking out mempool/electrs {}", tag));
...
fn install_targets(sh: &Shell, source_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for target in TARGETS {
        ...
        cmd!(sh, "rustup target add {triple}")
...
fn build_target(
    ...
    match target.builder {
        ...
        Builder::Cross => {
            ...
            command = command.env_remove("RUSTUP_TOOLCHAIN");
...
/// Make sure that each published archive contains only the `electrs` binary from mempool/electrs.
