...
//! Build local release archives for `Blockstream/electrs`.
//!
...
// `Blockstream/electrs` commit that the crate downloads.
include!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/src/indexer/blockstream_electrsd/versions.rs"
));
...
/// `Blockstream/electrs` repository used as the release source.
const BLOCKSTREAM_ELECTRS_REPO: &str = "https://github.com/Blockstream/electrs";
...
/// All binaries published for a single Blockstream/electrs commit.
const TARGETS: &[Target] = &[
    Target {
        triple: "aarch64-apple-darwin",
        artifact_name: "blockstream-electrs-darwin-arm64.tar.gz",
        exe_name: "electrs",
    ...
    Target {
        ...
        artifact_name: "blockstream-electrs-darwin-amd64.tar.gz",
        exe_name: "electrs",
        builder: Builder::Cargo,
        bindgen_args: None,
    },
    Target {
        ...
        artifact_name: "blockstream-electrs-linux-amd64.tar.gz",
        exe_name: "electrs",
    ...
    Target {
        ...
        artifact_name: "blockstream-electrs-linux-arm64.tar.gz",
        exe_name: "electrs",
        builder: Builder::Cross,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let compile_blockstream_electrs_dir =
        manifest_dir.join("contrib/bins/compile_blockstream_electrs");
    let cross_config = compile_blockstream_electrs_dir.join("Cross.toml");
    let dist_dir = compile_blockstream_electrs_dir
        .join("dist")
        .join(format!("blockstream-electrs-{BLOCKSTREAM_ELECTRS_VERSION}"));
    let workdir = compile_blockstream_electrs_dir.join("tmp");
    let source_dir = workdir.join("electrs");

    // Fail up front with actionable messages before spending time cloning or
    // compiling the Blockstream/electrs checkout.
    log_step("checking required tools");
    require_tools(&["git", "cargo", "rustup", "tar"])?;
    ...
    println!(
        ...
        dist_dir
            .join(format!(
                "blockstream-electrs-{}-SHA256SUMS",
                BLOCKSTREAM_ELECTRS_VERSION
            ))
            .display()
    );
...
/// Clone or reuse the upstream repository and reset it to the pinned commit.
fn prepare_source(sh: &Shell, source_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    let source_dir_s = path_string(source_dir);

    if source_dir.exists() {
        log_step(format!(
            "using existing Blockstream/electrs checkout {}",
            source_dir.display()
        ));
    } else {
        log_step(format!(
            "cloning Blockstream/electrs {} into {}",
            BLOCKSTREAM_ELECTRS_VERSION,
            source_dir.display()
        ));
        let repo = BLOCKSTREAM_ELECTRS_REPO;
        cmd!(sh, "git clone {repo} {source_dir_s}").run_echo()?;
    }

    let sh = sh.with_current_dir(source_dir);
    log_step("fetching Blockstream/electrs refs");
    cmd!(sh, "git fetch --tags --force").run_echo()?;

    let commit = BLOCKSTREAM_ELECTRS_VERSION;
    log_step(format!("checking out Blockstream/electrs {}", commit));
    cmd!(sh, "git fetch origin {commit}").run_echo()?;
    cmd!(sh, "git checkout --force {commit}").run_echo()?;
    let head = cmd!(sh, "git rev-parse HEAD").read()?;
    if head.trim() != commit {
        return Err(format!("expected commit {commit}, got {head}").into());
    }
...
fn install_targets(sh: &Shell, source_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    let sh = sh.with_current_dir(source_dir);
    for target in TARGETS {
...
fn build_target(
    ...
    match target.builder {
        Builder::Cargo => {
            ...
            cmd!(
                sh,
                "cargo build --locked --release --bin electrs --target {triple}"
            )
            .run_echo()?;
        }
        Builder::Cross => {
            ...
            let mut command = cmd!(
                sh,
                "cross build --locked --release --bin electrs --target {triple}"
            );
            ...
            // Cross.toml links the distro-provided libclang into a stable path so this does not
            // depend on the LLVM version installed by the base image.
            command = command.env("LIBCLANG_PATH", "/opt/halfin/libclang");
            command = command.env("CLANG_PATH", "/usr/bin/clang");
...
/// Make sure that each published archive contains only the `electrs` binary from
/// Blockstream/electrs.
fn verify_archive(artifact: &Path, exe_name: &str) -> Result<(), Box<dyn std::error::Error>> {
    let entries = output(Command::new("tar").arg("-tzf").arg(artifact))?
        .lines()
